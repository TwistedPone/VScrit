/*
Borrowed from: TheCherno's ImGui Vulkan Walnut Application.cpp
...
static VScritImgui::Render* s_Instance = nullptr;
...
namespace VScritImgui {
    ...
    void Render::Init()
    {
        ...
        m_WindowHandle = SDL_CreateWindow("Dear ImGui SDL3+Vulkan example", (int)(1280 * main_scale), (int)(720 * main_scale), window_flags);
        if (m_WindowHandle == nullptr)
        {
            std::cerr << ("Error: SDL_CreateWindow(): %s\n", SDL_GetError());
            return;
        }
        ...
        // VkResult isn't being used for SDL_Vulkan_CreateSurface, need to investigate if 
        // this is needed anymore or needs to be fixed!
        VkSurfaceKHR surface;
        VkResult err;
        if (SDL_Vulkan_CreateSurface(m_WindowHandle, g_Instance, g_Allocator, &surface) == 0)
        {
    ...
    // Upload Fonts
    // TODO: Create the Font script here
    ...
    void Render::Shutdown()
    {
        ...
        // Wait until all operations are finished before destroying everything
    ...
    VkInstance Render::GetInstance()
    {
        return g_Instance;
    }

    VkPhysicalDevice Render::GetPhysicalDevice()
    {
        return g_PhysicalDevice;
    }

    VkDevice Render::GetDevice()
    {
        return g_Device;
    }

    VkCommandBuffer Render::GetCommandBuffer(bool begin)
    {
        ImGui_ImplVulkanH_Window* wd = &g_MainWindowData;
    ...
    void Render::FlushCommandBuffer(VkCommandBuffer commandBuffer)
    {
        const uint64_t DEFAULT_FENCE_TIMEOUT = 100000000; // 100ms
    ...
    void Render::SubmitResourceFree(std::function<void()>&& func)
    {
        s_ResourceFreeQueue[s_CurrentFrameIndex].push_back(func);
    }
}
